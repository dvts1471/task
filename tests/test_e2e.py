...
class TestE2E:
    ...
    @allure.description(
        "This test verifies that user can place bet successfully and balance is updated accordingly"
    )
    ...
    def test_successful_bet_placement(self, driver: None, reset_balance: None) -> None:
        ...
        with allure.step(
            'Fill positive number (in range 1<=x<=100) to "Stake" input field'
        ):
            stake = 10
        ...
        with allure.step(
            "Verify that balance has been decreased by the amount of stake"
        ):
            verify(
